...
const verifierEmail = async (email: string) => {
  const response = await fetch(
    `http://localhost:8080/api/user/verifer-email?email=${email}`,
    {
      method: "GET",
      headers: {
  ...
  if (!response.ok) {
    throw new Error("Conflict Data ,this Email not  exists");
  }
  return response.json();
};
const registerFn = async ({ ...data }: TypeData) => {
  const response = await fetch(
    "http://localhost:4000/api/authentication/sign-up",
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
...
const SignUP = () => {
  ...
  const { mutate } = useMutation({
    ...
    onSuccess: (data) => {
      ...
      setTimeout(() => {
        ...
        if (
          data.message === "Principale created successfully" ||
          data.message === "Both created successfully"
        ) {
  ...
  const { data } = useQuery({
    queryKey: ["email-valid", state.email],
    queryFn: ({ queryKey }) => {
      const [, email] = queryKey;
      return verifierEmail(email as string);
    },
    enabled: !!state.email,
  });
  ...
  return (
    <div className="w-full h-screen flex justify-center items-center bg-[#f2f4f8] ">
      <form
      ...
      >
        ...
        <div className=" relative">
          <input
            ...
            onChange={handleChange}
            className={`border border-gray-400 outline-none  py-2.5 px-4 rounded-lg w-full`}
          />
          ...
          <div className="w-full flex items-center justify-center gap-6">
            {["Male", "Female"].map((gender) => (
              <label
                htmlFor={gender}
                key={gender}
                className={`basis-1/2 border border-gray-400 flex items-center justify-between px-4 py-1.5 rounded-lg ${
                  state.sexe === gender ? "bg-slate-200" : ""
                }`}
              >
        ...
        <button
          className="bg-blue-600 text-white py-2.5 rounded-lg hover:bg-blue-800 transform duration-300 focus:scale-[0.99] shadow"
          type="submit"
        >
      ...
      <div
        className={`py-6 px-4 rounded-md bg-white text-nowrap flex items-center gap-4 shadow drop-shadow absolute ${
          isError ? "bottom-10" : "-bottom-40 hidden"
        } `}
      >
        <BiSolidMessageRoundedError className=" text-red-500 text-3xl" />
        <p> Email a été utilisé ou n'est pas valide.</p>
      </div>
    </div>
  );
};
