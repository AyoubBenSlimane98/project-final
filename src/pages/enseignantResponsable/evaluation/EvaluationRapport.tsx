...
const getBinomeData = async ({
  ...
  const response = await fetch(
    `http://localhost:4000/api/eutdaint/binomes/${idB}`,
    {
      method: "GET",
      headers: {
        "Content-Type": "application/json",
...
const EvaluationRapport = () => {
  ...
  const handleNext = (id: number) => {
    ...
    if (accessToken) {
      ...
      setNextstep(true);
      navigate("/ens-responsable/evaluation-binome");
    }
  };
  useEffect(() => {
    if (groupes && groupe) {
  ...
  return (
    <main className="flex flex-col items-center pt-6 w-full min-h-svh mx-auto  px-12 relative mt-20 ">
      ...
      <div
        className={`bg-white shadow-md max-h-[595px] w-[90%]   rounded-lg  overflow-auto`}
      >
                  ...
                  >
                    <div
                      className={`cursor-pointer select-none flex items-center ${
                        header.column.getCanSort() ? "hover:text-gray-300" : ""
                      }`}
                    ...
                    if (
                      ...
                      return isFirstOccurrence ? (
                        <td
                          key={cell.id}
                          rowSpan={rowSpan}
                          className=" px-6 py-4 whitespace-nowrap text-sm text-gray-500 font-bold text-center align-middle border border-b-0 border-t-0 border-gray-300"
                        >
                          {flexRender(
...
export default EvaluationRapport;
