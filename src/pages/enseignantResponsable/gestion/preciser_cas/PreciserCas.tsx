...
import { CustomSelectGroupeProps, Groupe } from '../../consultation/GroupeBinome';
...
const setCasOfSujet = async ({
    ...
    const response = await fetch(`http://localhost:4000/api/responsable/set-cas/${idG}`, {
        method: "POST",
        headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${accessToken}`,
    ...
    return (
        <div className="bg-white shadow border outline-none border-gray-200 w-full px-1 py-1.5 flex items-center justify-between gap-5 rounded-md transform duration-200 ease-in-out transition-all cursor-pointer">
            ...
            <div className="flex items-center gap-3 px-2">
                {isEdit ? (
                    <RiSave3Line className="text-green-600" onClick={() => { handleEdit(); updateCas(name, input) }} />
                ) : (
                    <FaUserEdit className="text-blue-600" onClick={handleEdit} />
                )}
                <RiDeleteBin5Fill className="text-red-600" onClick={() => deleteCas(name)} />
            </div>
        </div>
    );
}
const PreciserCas = () => {
    ...
    const { allCas, createCas, next, setNext, actor, setActor, restAll } = useCasStore(useShallow((state) => ({
        allCas: state.allCas,
    ...
    const handleSend = () => {
        ...
        if (accessToken && actor && allCas.length > 0 && groupe) {
            ...
            mutate({ idG: groupe, accessToken, data: { acteur: actor, cas: allCas } })
        }
    }
    return (
        <section className='className="w-full h-svh px-6 pb-10 flex flex-col justify-center items-center gap-4 md:gap-6 sm:px-10 sm:py-6 overflow-hidden bg-[#F4F7FD] relative"'>
            {next ? (
                <div className="w-xl  bg-white  flex flex-col  drop-shadow-md shadow px-6 py-8 rounded-md">
                    ...
                    <div className='w-full mb-6'>
                        {groupes && <CustomGroupSelect responsable={groupes} label='Selection un groupe : ' />}
                        <div className="space-y-1">
                            ...
                            <input
                                ...
                                className={`w-full py-2 px-4 border  outline-none rounded-md placeholder:text-sm focus:border-2 focus:border-sky-600 focus:outline-2 focus:outline-sky-600 transform duration-200 ease-in transition-all ${errorActor ? "border-red-600" : 'border-gray-300'}`}
                            />
                        </div>
                ...
                <section className=" w-2xl md:basis-1/3 bg-white  border border-gray-200 rounded-md px-6 py-6 flex flex-col gap-y-4 shadow shadow-gray-200">
                    <div className="">
                        <label htmlFor="" className="block font-medium text-xl mb-4">Cas d'utilisation </label>
                        <div className="flex items-center justify-between gap-4 mb-4">
                            <input
                                ...
                                className={`w-full py-2 px-4 border  outline-none rounded-md  placeholder:text-sm focus:border-2 focus:border-sky-600 focus:outline-2 focus:outline-sky-600 transform duration-200 ease-in transition-all ${errorCas ? "border-red-600" : 'border-gray-300'}`} />
                            <div className=" shrink-0 w-14 h-10 border border-gray-300 rounded-md flex items-center justify-center cursor-pointer  hover:bg-gray-100 " onClick={() => hanleCas(cas!)} >
                        ...
                        </div>
                        {
                            <div className=" w-full h-48  px-2 py-3 overflow-hidden flex flex-col gap-y-2.5 border border-gray-200 rounded-md hover:overflow-auto hover:bg-gray-50  cursor-pointer ">
                                {allCas.map((ref, index) => <CardInfoRef key={index} name={ref} />)}
                            </div>
                        }
                    </div>
                    <div className="w-full flex items-center justify-center gap-x-6">
                        ...
                        <button
                            ...
                            onClick={handleSend}
                            type="button"
                            className={` ${allCas.length === 0 && !actor ? ' cursor-not-allowed' : 'cursor-pointer'} outline-none basis-1/2 border-none bg-blue-400 hover:bg-blue-600 rounded-md py-2 text-white font-medium transition-all}`}
                        >
            ...
            <div className={`py-6 px-4 rounded-md bg-white text-nowrap flex items-center gap-4 shadow drop-shadow absolute ${isSucces ? "bottom-10" : "-bottom-20 hidden"} `}>
                <FaCheckCircle className=" text-green-500 text-3xl" />   <p>Les cas ont été créés avec succès.</p>
            </div>
        </section>
    )
