  ...
  return (
    <div className="bg-white shadow border outline-none border-gray-200 w-full p-0.5 flex items-center justify-between gap-5 rounded-md transform duration-200 ease-in-out transition-all cursor-pointer">
      ...
      <div className="flex items-center gap-3 px-2">
        ...
        <RiDeleteBin5Fill
          className="text-red-600"
          onClick={() => deletePrerequis(name === input ? name : input)}
        />
      </div>
    </div>
  );
...
const DecrirSujet = () => {
  ...
  const { mutate } = useMutation({
    mutationFn: ({
      ...
      accessToken: string;
      createSujet: CreateSujet;
    }) => createdSujet({ accessToken, createSujet }),
    onSuccess: () => {
      setIsLoading(true);
      setTimeout(() => {
        setIsLoading(false);
    ...
    if (accessToken) {
      mutate({
        ...
        createSujet: {
          ...
          references: refInputSuject.map((ref) => ({ reference: ref })),
          prerequises: prerequisInputSuject.map((prerequis) => ({ prerequis })),
        },
      });
    }
  };
  return (
    <main className="w-full h-full md:h-svh mt-20 p-6 flex flex-col mx-auto items-center gap-y-6 lg:mt-26  ">
      ...
      <section className="w-full flex flex-col md:flex-row  justify-evenly items-center gap-2  md:border md:border-gray-200  md:h-[400px]  md:px-4 lg:mb-4 lg:gap-4 lg:px-8">
        ...
        <section className=" w-full md:basis-1/3  h-80 border border-gray-200 rounded-md px-4 py-6 flex flex-col gap-y-4 shadow shadow-gray-200">
          <div className="space-y-1">
            ...
            <div className="flex items-center justify-between gap-4 mb-4">
              ...
              />
              <div
                className=" shrink-0 w-10 h-10 border border-gray-400 rounded-md flex items-center justify-center cursor-pointer  hover:bg-gray-100 "
                onClick={() => {
            ...
            {refInputSuject.length > 0 && (
              <div className=" w-full h-48  p-2 overflow-hidden flex flex-col gap-y-2 border border-gray-200 rounded-md hover:overflow-auto hover:bg-gray-50  cursor-pointer ">
                {refInputSuject.map((ref, index) => (
        ...
        <section className="w-full md:basis-1/3  h-80 border border-gray-200 rounded-md px-4 py-6 flex flex-col gap-y-4 shadow shadow-gray-200">
          <div className="space-y-1">
            ...
            {prerequisInputSuject.length > 0 && (
              <div className=" w-full h-48  p-2 overflow-hidden flex flex-col gap-y-2 border border-gray-200 rounded-md hover:overflow-auto hover:bg-gray-50 cursor-pointer ">
                {prerequisInputSuject.map((prev: string, index: number) => (
                  <CardInfoPrerquis key={index} name={prev} />
                ))}
              </div>
            )}
          </div>
      ...
      <div className="w-full  py-4 flex items-center justify-center gap-x-6 ">
        <button
          className=" outline-none w-96 bg-blue-500 hover:bg-blue-700 rounded-md py-2.5 text-white font-medium transform duration-200 ease-in-out transition-all cursor-pointer"
          onClick={handleSumit}
        >
          {isLoading ? "Créer..." : "Créer"}
        </button>
        <button
          className=" outline-none w-96 bg-red-500 hover:bg-red-700  rounded-md py-2.5 text-white font-medium transform duration-200 ease-in-out transition-all cursor-pointer"
          onClick={() => {
            resetAll();
            setForm({ nom: "", description: "" });
          }}
      ...
      >
        ...
        <p>le sujet été affectés avec succès.</p>
      </div>
    </main>
  );
};
