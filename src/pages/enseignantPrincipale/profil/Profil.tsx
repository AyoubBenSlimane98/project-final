    ...
    return (
        <div className="w-full px-6 py-8 shadow-lg rounded-lg border border-gray-200 bg-white">
            ...
            <div className="flex flex-col space-y-4">
                ...
                />
                <button
                    className={`flex items-center justify-center sm:hidden space-x-2 text-white py-1.5 px-2.5 sm:px-4 text-center sm:py-2 rounded-full ${!isEditing ? "bg-sky-500" : "bg-green-500"
                        }`}
                    onClick={() => setIsEditing((prev) => !prev)}
                >
                    {!isEditing ? (
    ...
    return (
        <div className="w-full px-6 py-4  shadow-lg rounded-lg border border-gray-200 bg-white">
            <div className="flex justify-between items-center ">
                ...
                <button
                    className={`flex items-center space-x-2 text-white  text-center py-1 px-2.5 sm:py-2 sm:px-4 rounded-full ${!isEditing ? "bg-sky-500" : "bg-green-500 hidden sm:flex"
                        }`}
                    onClick={() => setIsEditing((prev) => !prev)}
                >
                    {!isEditing ? (
            ...
            {isEditing && (
                <div className="w-full flex flex-col space-y-4 mt-4 mb-2">
                    ...
                    <button
                        className={`flex items-center justify-center sm:hidden space-x-2 text-white px-4 text-center py-2 rounded-full ${!isEditing ? "bg-sky-500" : "bg-green-500"
                            }`}
                        onClick={() => setIsEditing((prev) => !prev)}
                    >
                        {!isEditing ? (
