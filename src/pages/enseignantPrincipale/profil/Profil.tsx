...
const updateImage = async ({
  ...
  const response = await fetch(
    "http://localhost:4000/api/principal/profil/img",
    {
      method: "PUT",
      headers: {
        "Content-Type": "application/json",
...
const updateInfoProfil = async ({
  ...
  const response = await fetch(
    "http://localhost:4000/api/principal/profil/info",
    {
      method: "PUT",
      headers: {
        "Content-Type": "application/json",
...
const updatePasswordProfil = async ({
  accessToken,
  form,
}: {
  accessToken: string;
  ...
  const response = await fetch(
    "http://localhost:4000/api/admin/profil/password",
    {
      method: "PUT",
      headers: {
        "Content-Type": "application/json",
        ...
        <div className=" shrink-0 w-24 h-24 relative">
          <img
            onError={(e) => {
              ...
              target.onerror = null;
              target.src = "https://i.sstatic.net/l60Hf.png";
            }}
  ...
  const inputClasses = ` ${
    status
      ? isEmail
        ...
        : error
        ? "border-red-600 outline-none border rounded px-2 py-1.5"
...
function MyInformation({ dataProfil, accessToken, refetch }: MyProfileProps) {
  ...
  const [isEditing, setIsEditing] = useState<boolean>(false);
  const [data, setData] = useState<MyInfoProps>({
    bio: "",
    nom: "",
    prenom: "",
  });
  ...
  const { mutate } = useMutation({
    mutationFn: ({
      ...
      accessToken: string;
      data: MyInfoProps;
    }) => updateInfoProfil({ accessToken, data }),
    onSuccess: (data) => {
      console.log(data);
      refetch();
    },
  ...
  return (
    <div className="w-full px-6 py-8 shadow-lg rounded-lg border border-gray-200 bg-white">
      ...
      <div className="flex flex-col space-y-4">
        ...
        <MyInput name="email" label="Email" value={email} status={isEditing} />
      </div>
    </div>
  );
}
function MyPassword({
  ...
  const handleChangePassword = () => {
    if (data.confirmPassword === data.newPassword) {
      mutate({
        accessToken,
        form: { oldPassword: data.oldPassword, newPassword: data.newPassword },
      });
    } else {
      setError(true);
...
const Profil = () => {
  ...
  return (
    <div className="w-full h-svh  px-6 pb-10 flex flex-col gap-4 md:gap-6 sm:px-10 sm:py-6 overflow-auto bg-[#F4F7FD] relative">
      ...
      <div
        className={` absolute ${
          isSucces ? "bottom-10" : "hidden"
        }  left-1/2 -translate-x-1/2  w-xl py-4  rounded-md bg-white drop-shadow shadow flex items-center justify-center gap-4`}
      >
