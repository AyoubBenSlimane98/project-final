...
const createGroupes = async ({
  ...
  accessToken: string;
  id: number;
  users: BinomeWithStudents;
}) => {
  const response = await fetch(
    `http://localhost:4000/api/principal/create-groupe/${id}`,
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
...
const CreerGroupe = () => {
  ...
  const { data: binomesData } = useQuery({
    queryKey: ["binomes"],
    queryFn: getListBinommes,
  });
  ...
  const { mutate } = useMutation({
    mutationFn: ({
      ...
      id: number;
      users: BinomeWithStudents;
    }) => createGroupes({ accessToken, id, users }),
    onSuccess: () => {
      setLoading(true);
      setTimeout(() => {
        setLoading(false);
  ...
  const handleSubmit = () => {
    if (accessToken && users) {
      mutate({ accessToken, id: value, users });
    }
  };
  return (
    <section className="w-full h-svh px-6 pb-10 flex flex-col justify-center items-center gap-4 md:gap-6 sm:px-10 sm:py-6 overflow-hidden bg-[#F4F7FD] relative">
      <div className="w-[580px] h-72 bg-white rounded-[20px] flex flex-col justify-center items-center drop-shadow-md shadow ">
        ...
        <div
          className={`${
            count > 0 && value > 1 ? " justify-between" : "justify-end "
          } w-full px-11 flex items-center py-2   mb-2 `}
        >
          {value !== 1 && (
            <p className="text-sm">
              {" "}
              Il sera créé <span className="text-blue-500">{value}</span>{" "}
              groupes{" "}
              <span
          ...
          <NavLink
            to="/ens-principale/creer-groupes/liste-binomes"
        ...
        {isOpen && (
          <ul className="space-y-1 px-4 py-4 absolute w-80 bg-white z-50 shadow drop-shadow rounded-md  ">
            <div className="w-full flex items-center justify-end">
              <IoCloseOutline
                onClick={() => setIsOpen(false)}
                className=" text-xl hover:text-red-500 transform duration-200 ease-in-out transition"
              />
            </div>
            {groupes.map((groupe, idx) => (
              <li
                key={idx}
                className="text-sm text-gray-700 hover:bg-slate-100 py-0.5 px-2 transform duration-200 ease-in-out transition"
              >
                <p>
                  {groupe.name} : <strong>{groupe.binomes}</strong> binôme
                  {groupe.binomes > 1 ? "s" : ""}
                </p>
              </li>
            ))}
      ...
      <div
        className={`py-6 px-4 rounded-md bg-white text-nowrap flex items-center gap-4 shadow drop-shadow absolute ${
          isError ? "bottom-10" : "-bottom-20"
        } `}
      >
        <PiWarningFill className=" text-yellow-500 text-3xl" />{" "}
        <p>Désolé, Les groupes ont déjà été créés.</p>
      </div>
      <div
        className={`py-6 px-4 rounded-md bg-white text-nowrap flex items-center gap-4 shadow drop-shadow absolute ${
          isSucces ? "bottom-10" : "-bottom-20"
        } `}
      >
        <FaCheckCircle className=" text-green-500 text-3xl" />{" "}
        <p>Les groupes ont été créés avec succès.</p>
      </div>
    </section>
  );
};
